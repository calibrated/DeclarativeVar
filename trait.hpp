...
template <typename T>
struct ContainerTrait<T, std::enable_if_t<!is_shared_ptr<T>::value && !is_variable<T>::value>>
{
...
template <typename T>
struct ElementTrait<T, std::enable_if_t<!is_shared_ptr<T>::value && !is_variable<T>::value>>
{
    using type= T;
